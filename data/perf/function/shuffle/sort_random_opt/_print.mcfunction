#perf:shuffle/sort_random_opt/_print
# 输出测试数据

data modify storage perf:io temp set from storage perf:class shuffle/sort_random_opt

tellraw @a "--- shuffle/sort_random_opt perf result ---"
...
tellraw @a ["scb_add: ",{"score":{"name":"perf_scb_add","objective":"int"}}]

# 超过阈值重测
execute if score perf_err int <= perf_check int run return 0
tellraw @a "shuffle/sort_random_opt perf restart"
